...
if (isset($_POST['UpdateProduct'])) {
    ...
    if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])) {
        ...
        if (!empty($_FILES['prod_img']['name'])) {
            ...
            $prod_img = $_POST['existing_img'];
        }

        // Update query
        $postQuery = "UPDATE rpos_products SET prod_code =?, prod_name =?, prod_category=?, prod_img =?, prod_desc =?, prod_price =? WHERE prod_id = ?";
        $postStmt = $mysqli->prepare($postQuery);
        // Bind parameters
        $rc = $postStmt->bind_param('sssssss', $prod_code, $prod_name, $prod_category, $prod_img, $prod_desc, $prod_price, $update);
        $postStmt->execute();
                ...
                <form method="POST" enctype="multipart/form-data">
                  ...
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="hidden" name="existing_img" value="<?php echo $prod->prod_img; ?>">
                      <?php if ($prod->prod_img) { ?>
                        <img src="../admin/assets/img/products/<?php echo $prod->prod_img; ?>" height="60" width="60" class="img-thumbnail mt-2">
                      <?php } ?>
                    </div>
                    <div class="col-md-6">
                  ...
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="text" name="prod_category" class="form-control" value="<?php echo $prod->prod_category; ?>">
                    </div>
                    <div class="col-md-6" hidden>
                      <label></label>
                      <input type="text" name="" class="form-control" value="<?php echo $prod->prod_price; ?>">
                    </div>
                  </div>
                  <hr>
                  <div class="form-row">
